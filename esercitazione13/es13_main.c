...
int main() {				
	int p1[DIM] = {3, -4, -4, -6};	//Global and static variables are
	int n1 = 4;			//initialized to 0.
	int p2[DIM] = {3, 1, 4, 6};
	int n2 = 4;
	int pr[DIM]; 
	int *nr;
	...
	if(sum == 0){			//If the polynomial is null
		*nr = -1;
		...
		for(i = 0; i < (max); ++i){
			if(pr[(max - 1) - i] != 0){		//Print only coefficient != 0
		                if((pr[(max - 1) - i]) > 0)	//If positive printf "+". Only for beauty
		                        printf("+");
	        	        printf("%dx^%d ", pr[(max - 1)  - i], (max - i));
     			}
	   	}
	}
	printf("\n\n");
